...
class ventana_principal_usu(tk.Frame):
    ...
    def aplicacion(self) -> None:
        messagebox.showinfo(title="Hola!!", message="Esta aplicación permite administrar un parqueadero ...") # TODO: poner mensaje
    ...
    def acerca_de(self) -> None:
        messagebox.showinfo(title="Hola!!", message="Los autores de esta maravillosa aplicacion somos:\n-Alejandro(power ranger azul)\n-Sofia(power ranger rosada)\n-Katherine(power ranger amarilla)\n-Sara(power ranger blanca)\n-Sebastian(power ranger rojo)")


    def interfaz_inicio(self) -> None:
        interfaz_inicio = tk.Frame(self, highlightbackground="black", highlightthickness=2)
        interfaz_inicio.pack(side="top", fill="both", expand=True, padx=10, pady=10)

        texto_bienvenida = tk.Label(interfaz_inicio, text="Bienvenido", font=("Arial", 20))
 ...
 ya que el programa se encargara del resto ;)"
        ...
        texto_descripcion = LabelAjustable(interfaz_inicio, text=texto, font=("Arial", 12))
